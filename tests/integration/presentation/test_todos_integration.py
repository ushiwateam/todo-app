...
from app.todos.presentation.schema import TodosResponse, TodosUpdateRequest
...
def test_create_todo_authenticated(
    client, create_user, auth_headers, title, description
):
    todo_data = {
        "title": title,
    ...
    response = client.post("/todos/", headers=auth_headers(user), json=todo_data)
...
def test_modify_owned_todo(client, create_user, create_todo, auth_headers):
    ...
    new_todo = {"title": "new title", "description": "new desc"}
    response = client.put(
        f"/todos/{todo.id}", headers=auth_headers(user), json=new_todo
    )
...
def test_modify_unowned_todo(client, create_user, create_todo, auth_headers):
    ...
    user_2 = create_user(
        name="user2", email="user2@example.com", password="user2password"
    )
    todo = create_todo(user_1)
    new_todo = {"title": "new title", "description": "new desc"}
    response = client.put(
        f"/todos/{todo.id}", headers=auth_headers(user_2), json=new_todo
    )
...
def test_get_todos_pagination(
    client, create_user, create_todo, auth_headers, page, limit
):
    user = create_user()
    todos = [
        create_todo(user, title=f"title{i}", description=f"desc{i}") for i in range(10)
    ]
    response = client.get(
        "/todos", params={"page": page, "limit": limit}, headers=auth_headers(user)
    )
    assert response.status_code == 200
    res = response.json()
    assert "data" in res
