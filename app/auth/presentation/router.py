...
@router.post(
    ...
    responses={**EMAIL_ALREADY_REGISTERED_RESPONSE},
)
def register_route(user: UserRegisterRequest, auth_service: AuthServiceDep):
    try:
...
@router.post(
    "/login", name="Login an existent user", responses={**LOGIN_UNAUTHORIZED_RESPONSE}
)
def login_route(user: UserLoginRequest, auth_service: AuthServiceDep):
    try:
...
async def login_for_access_token(auth_service: AuthServiceDep, form_data: FormDataDep):
    return auth_service.token_login(
