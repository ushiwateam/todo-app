...
- `.githooks/pre-push` runs `black --check` and `isort --check-only` (also POSIX sh, no dependencies) on the `.py` files carried by the commits being pushed, and rejects the push if any is unformatted. It reads the ref lines git supplies on stdin: a new branch (`remote_sha` all zeros) resolves to `git rev-list <local> --not --remotes=<remote>` plus a `git diff-tree --root` per commit, otherwise `git diff <remote_sha> <local_sha>`; results are filtered to `.py` paths that still exist on disk. Both tools always run so one push reports everything, and a missing `black`/`isort` is a hard failure rather than a silent skip. Bypass with `git push --no-verify`.
- Formatter settings live in `pyproject.toml` (`line-length = 88`, isort `profile = "black"`) — the one place shared by the hook, the PyCharm External Tools and any future CI. It contains no `[tool.pytest.ini_options]`, so test discovery is unaffected — pytest now prints `configfile: pyproject.toml`, but rootdir, collection and results are identical (32 collected, 32 passed with and without the file).
- **The repo is not yet black/isort-clean** (~37 files fail black, ~19 fail isort as of the hook's introduction). That is why `pre-push` is scoped to pushed files instead of the whole repo — a repo-wide check would block every push until a bulk reformat lands. Don't add a repo-wide `black --check .` to CI before that reformat happens.
